...
if (!identical(Sys.getenv("FASTKPC_RUN_CUDA_TESTS", unset = "0"), "1")) {
  cat("SKIP Phase 4 artifact validation: FASTKPC_RUN_CUDA_TESTS != 1\n")
  quit(save = "no", status = 0L)
}
if (!identical(Sys.getenv("CUDA_VISIBLE_DEVICES", unset = ""), "0")) {
  fail("Phase 4 artifact validation requires physical GPU 0")
}
...
artifact_paths <- file.path(root, directories)
names(artifact_paths) <- names(directories)
if (!all(dir.exists(artifact_paths))) {
  evidence_paths <- c(
    Sys.getenv(
    ...
    Sys.getenv(
      ...
      unset =
        "/tmp/fastkpc-phase4-full-shadow-merged-precision-floor-v1.rds"
    ),
    Sys.getenv(
  ...
  assert_true(
    all(file.exists(evidence_paths)),
    "Phase 4 evidence inputs are required to generate missing artifacts"
  )
  status <- system2(
    "Rscript",
  ...
  assert_true(status == 0L, "Phase 4 artifact generation must succeed")
}

validated <- lapply(names(artifact_paths), function(kind) {
  fastkpc_full_cuda_phase4_validate_artifact(
    artifact_paths[[kind]], expected_kind = kind
  )
})
names(validated) <- names(artifact_paths)
validated_current <- lapply(names(artifact_paths), function(kind) {
  fastkpc_full_cuda_phase4_validate_artifact(
    artifact_paths[[kind]], expected_kind = kind,
    verify_current_sources = TRUE
  )
})
names(validated_current) <- names(artifact_paths)
assert_true(
  all(vapply(validated, function(value) isTRUE(value$summary$pass),
             logical(1L))) &&
    length(unique(vapply(
      validated, function(value) value$producer$native_binary_sha256,
      character(1L)
    ))) == 1L && length(unique(vapply(
      validated, function(value) value$producer$producer_source_closure_sha256,
      character(1L)
    ))) == 1L && all(vapply(names(validated), function(kind) {
      identical(
        validated[[kind]]$producer$identity_sha256,
        validated_current[[kind]]$producer$identity_sha256
      )
    }, logical(1L))),
  "Phase 4 artifacts must share one validated source and native authority"
)

copy_artifact <- function(kind) {
  destination <- tempfile(paste0("fastkpc-phase4-", kind, "-tamper-"))
  dir.create(destination)
  files <- list.files(artifact_paths[[kind]], full.names = TRUE)
  assert_true(
    all(file.copy(files, destination, overwrite = TRUE)),
    paste0("Phase 4 ", kind, " tamper fixture copy must succeed")
  )
  destination
}
...
cases$candidate_sp[[1L]] <- cases$candidate_sp[[1L]] * 2
utils::write.csv(cases, case_path, row.names = FALSE, na = "")
assert_error(
  fastkpc_full_cuda_phase4_validate_artifact(
    oracle_tampered, expected_kind = "oracle"
  ),
  "Phase 4 payload file hash mismatch: case_results.csv",
  "Phase 4 semantic payload tampering must fail before interpretation"
)
...
assert_error(
  ...
  "Phase 4 artifact manifest schema mismatch",
  "Phase 4 manifest claim-scope tampering must fail closed"
)
...
assert_error(
  ...
  "Phase 4 validator attestation tampering must fail closed"
)
...
assert_error(
  ...
  "Phase 4 volatile receipt tampering must fail closed"
)

cat(
  "PASS Phase 4 single-penalty GCV artifacts; producers=",
  paste(vapply(
