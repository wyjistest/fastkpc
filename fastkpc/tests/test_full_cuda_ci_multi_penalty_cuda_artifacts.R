...
assert_true(
  all(dir.exists(paths)), "Phase 6 artifacts must exist before validation"
)
...
assert_true(
  all(vapply(validated, function(value) isTRUE(value$summary$pass),
    ...
    isTRUE(validated$backend$summary$performance_gate),
  "Phase 6 artifacts must share source/native authority and performance"
)

copy_artifact <- function(kind) {
  destination <- tempfile(paste0("fastkpc-phase6-", kind, "-tamper-"))
  dir.create(destination)
  files <- list.files(paths[[kind]], full.names = TRUE)
  assert_true(
    all(file.copy(files, destination, overwrite = TRUE)),
    paste0("Phase 6 ", kind, " tamper fixture copy must succeed")
  )
  destination
}
...
assert_error(
  ...
  "Phase 6 payload file hash mismatch: case_results.csv",
  "Phase 6 semantic payload tampering must fail closed"
)
...
assert_error(
  ...
  "Phase 6 artifact manifest schema mismatch",
  "Phase 6 manifest claim-scope tampering must fail closed"
)
...
assert_error(
  ...
  "Phase 6 validator attestation tampering must fail closed"
)
...
assert_error(
  ...
  "Phase 6 volatile receipt tampering must fail closed"
)

cat(
  "PASS Phase 6 multi-penalty CUDA artifacts; producers=",
  paste(vapply(
