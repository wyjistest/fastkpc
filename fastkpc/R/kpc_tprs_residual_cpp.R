...
fastkpc_kpc_tprs_gcv_log_gradient <- function(
...
fastkpc_kpc_tprs_magic_boundary_probe <- function(
    ...
    direction = NULL,
    tol = sqrt(.Machine$double.eps),
    diagnostics_level = c("full", "compact")) {
  diagnostics_level <- match.arg(diagnostics_level)
  collect_diagnostics <- identical(diagnostics_level, "full")
  score_at <- function(lambda) {
    fastkpc_kpc_tprs_gcv_score_for_setup(
      y = y, setup = setup, lambda = lambda, tol = tol)
  }
  current <- selected
  current_lambda <- as.numeric(current$lambda)
  current_score <- as.numeric(current$gcv)
  if (!is.finite(current_lambda) || current_lambda <= 0 ||
    ...
    return(list(
      ...
      start_lambda = current_lambda,
      start_score = current_score,
      gradient = NA_real_,
      path = if (isTRUE(collect_diagnostics)) data.frame() else NULL
    ))
  }
  log_step <- as.numeric(log_step)[1L]
  if (!is.finite(log_step) || log_step <= 0) log_step <- 2
  max_steps <- as.integer(max_steps)[1L]
  ...
  direction <- if (!is.null(direction) && length(direction) > 0L &&
    ...
    up <- score_at(current_lambda * exp(log_step))
    down_score <- as.numeric(down$gcv)
    up_score <- as.numeric(up$gcv)
    if (is.finite(down_score) &&
        down_score < current_score - improvement_tol &&
        (!is.finite(up_score) || down_score <= up_score)) {
      -1L
    } else if (is.finite(up_score) &&
               up_score < current_score - improvement_tol) {
      1L
    } else {
      0L
    }
  }
  path <- if (isTRUE(collect_diagnostics)) list() else NULL
  moved <- FALSE
  steps <- 0L
  if (direction != 0L && max_steps > 0L) {
    for (i in seq_len(max_steps)) {
      candidate_lambda <- current_lambda * exp(direction * log_step)
      candidate <- score_at(candidate_lambda)
      candidate_score <- as.numeric(candidate$gcv)
      accepted <- is.finite(candidate_score) &&
        candidate_score < current_score - improvement_tol
  ...
  list(
    ...
    start_lambda = as.numeric(selected$lambda),
    start_score = as.numeric(selected$gcv),
    gradient = as.numeric(grad),
    path = if (!isTRUE(collect_diagnostics)) {
      NULL
...
fastkpc_kpc_tprs_magic1d_candidate <- function(
  ...
  step_failed <- FALSE
  last_iteration <- 0L
  trace <- if (isTRUE(collect_diagnostics)) {
    list(data.frame(
      iteration = 0L,
      phase = "start",
      lambda = exp(rho),
  ...
  for (iteration in seq_len(max_iter)) {
    last_iteration <- as.integer(iteration)
    try_count <- 0L
    accepted <- NA
    step_type <- NA_character_
    raw_step <- NA_real_
  ...
  if (isTRUE(collect_diagnostics)) {
    ...
    trace_df <- NULL
    grid <- NULL
    bracket <- range(c(lambda_start, current$lambda, selected$lambda),
                     finite = TRUE)
    iterations <- as.integer(last_iteration)
  }
  list(
    selected = selected,
...
fastkpc_kpc_tprs_local_gcv_candidate <- function(
  ...
  list(
    ...
    bracket = exp(bracket),
    lambda_start = as.numeric(lambda_start),
    pre_boundary_lambda = as.numeric(exp(opt$minimum)),
    pre_boundary_score = as.numeric(opt$objective),
    magic_boundary = boundary
  )
}
...
fastkpc_kpc_tprs_gcv_candidate <- function(
  ...
  if (identical(selection, "mgcv-magic")) {
    ...
    grid <- local$grid
    selected_grid_index <- if (is.data.frame(grid) && nrow(grid) > 0L) {
      which.min(abs(log(grid$lambda) - log(selected_lambda)))
    } else {
      NA_integer_
    }
    ...
    if (isTRUE(collect_diagnostics)) {
      ...
      grid <- list(
        lambda = vapply(grid_eval, `[[`, numeric(1L), "lambda"),
        rss = vapply(grid_eval, `[[`, numeric(1L), "rss"),
        edf = vapply(grid_eval, `[[`, numeric(1L), "edf"),
        gcv = vapply(grid_eval, `[[`, numeric(1L), "gcv"),
        valid = vapply(grid_eval, `[[`, logical(1L), "valid")
      )
    }
    if (!any(grid$valid)) {
  ...
  selected$grid <- if (isTRUE(collect_diagnostics)) grid else NULL
  selected$selected_grid_index <- as.integer(selected_grid_index)
  selected$gcv_grid_points <- if (isTRUE(collect_diagnostics) &&
                                  is.data.frame(grid)) {
    nrow(grid)
  } else if (identical(selection, "global-grid")) {
    length(lambda_grid)
  } else {
    0L
  }
  ...
  selected$diagnostics <- c(
    ...
    list(
      ...
      magic1d_trace = if (identical(selection, "mgcv-magic")) {
        if (isTRUE(collect_diagnostics)) list(local$trace) else NULL
      } else {
        if (isTRUE(collect_diagnostics)) list(data.frame()) else NULL
      },
...
fastkpc_kpc_tprs_magic_parity_diagnostics <- function(
  ...
  data.frame(
    ...
    local_pre_boundary_lambda =
      as.numeric(local$diagnostics$pre_boundary_lambda %||% NA_real_),
    local_magic_boundary_probe =
      isTRUE(local$diagnostics$magic_boundary_probe),
    local_magic_boundary_steps =
      as.integer(local$diagnostics$magic_boundary_steps %||% NA_integer_),
    local_magic_boundary_direction =
      as.integer(local$diagnostics$magic_boundary_direction %||% NA_integer_),
