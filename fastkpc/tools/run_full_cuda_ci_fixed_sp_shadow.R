...
executor <- function(context, shard_id, setup_keys, target_rows) {
  shard_logical <- logical_tests[
    logical_tests$shard_id == shard_id, , drop = FALSE
  ]
  rownames(shard_logical) <- NULL
