...
>
> **Strict-method optimization pass:** A route-authenticated one-call residual slab, persistent method CUDA context, and 256-thread HSIC row kernel retain every original miss cohort and gather only complete all-hit residual batches D2D. Conditioning systems through two penalties use qualified augmented QR; larger systems retain authoritative stable SVD. `hsic.perm` keeps a bounded 384 MiB deterministic component LRU and exact R 4.4 Rejection index generation. Commit `830cfbe` adds the authoritative fixed-SP root cache and opt-in original-window setup/optimizer pipeline to all strict methods. Its C++17 move-only permutation builder owns the table, computes OpenSSL SHA-256 incrementally, seals the payload without a mutable consumer alias, and eliminates the trusted consumer's second full payload scan; untrusted boundaries still rehash. Clean default-Inf 351x48 outer times are `593.814/849.693/1042.358` seconds for `hsic.gamma/dcc.perm/hsic.perm`, down from `631.522/945.359/1152.905` at `50595ce`; the combined time falls from `2729.786` to `2485.865` seconds (`8.94%`). Physical residual fits remain `194,687/189,896/189,258`. `hsic.gamma` retains maximum p-value difference `8.271161533457416e-14` under its `1e-10` contract with zero decision flips; both permutation routes remain bitwise exact, including final R RNG state. All 834,467 skeleton tests, 19 kpcalg-authority orientation CI tests, traces, graph outputs, and final PDAGs pass; CPU numerical authority, fallback, residual/component D2H, residual-cache eviction, root-cache bypass, and root identity rejection are zero. The clean native SHA-256 is `443e2538e6f2030bbd071fa769a53b8e397c3cb73bc5b38c55fc6393c5eafbd3`; evidence is indexed by `fastkpc/artifacts/strict_ci_methods_351x48_optimized_v2/manifest.json`. This does not change the `dcc.gamma` Phase 10 candidate, sealed holdout, or promotion state.
>
> **Strict-method remaining opportunity:** The optimized routes are not at a hardware ceiling. Authoritative fixed-SP residual solving still costs `383.800/455.173/433.361` seconds for `hsic.gamma/dcc.perm/hsic.perm`, dominated by level-3-and-deeper stable SVD. Permutation generation plus incremental digest costs `117.275` seconds for `dcc.perm` and `255.772` seconds for `hsic.perm`; trusted identity build plus validation is now only `1.469/1.764` seconds. GPU residual/component preparation is still serial with R permutation generation, so the next lower-risk route is a genuinely asynchronous, one-in-flight-batch pipeline that preserves exact R RNG and delayed-error semantics. A qualified deeper-level residual kernel remains the shared high-impact route. Mixed-hit masking remains diagnostic-only, and more scheduler, target-cache, or outer grouped-QR work remains stopped by existing evidence.
>
