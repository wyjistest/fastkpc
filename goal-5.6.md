...
>
> **Current accepted implementation/evidence snapshot:** Phase 9 is COMPLETE. The best default-`Inf` fresh-data development run is now `219.513` seconds with the CPU-only setup/optimizer pipeline and a fixed 5 ms optimizer head start; the best retained non-pipelined default-`Inf` run is `233.848` seconds and the best explicit-max-7 run is `235.058` seconds. Per-call univariate primitive reuse, score-first line-search evaluation, authoritative fixed-SP penalty-root caching, and the original-window pipeline preserve bitwise-identical consumed p-values, exact graph/sepset/count/trace parity, and zero CPU numerical authority, fallback, or large-payload D2H. Checkpoint A passes, but Checkpoint B (`<180` seconds) and the final five-run median gate (`<=120` seconds) still fail. No candidate is frozen; Phase 10 remains ACTIVE, the external promotion holdout remains `SEALED_NOT_RELEASED`, and the route remains unrecommended.
>
> **Latest authoritative fixed-SP root cache:** Directly reusing the optimizer Cholesky root was rejected because it changed 5,991 of 220,859 level-3 p-values in low bits despite zero decision flips (`STOP_PRECOMPUTED_OPTIMIZER_ROOT_NUMERICAL_PARITY`). The retained route caches only the root produced and validated by the authoritative fixed-SP eigensolver, keyed by coefficient dimension, block offset, declared rank, dimension, and every penalty-block bit. Each one-call runtime is bounded to 4,096 entries and 256 MiB; a full cache bypass leaves the original root computation and result unchanged. Level 3 fell from `118.729` to `114.038` seconds. The full explicit-max-7 run fell from `249.539` to `235.058` seconds, with native setup falling from `39.200` to `23.642` seconds. Default Inf fell from `252.616` to `233.848` seconds. The default-Inf receipt records 28,611 lookups, 26,925 hits, 1,686 inserts, zero bypasses, zero identity rejections, and 8,691,624 device bytes. All task and p-value bits, graph fields, level semantics, and optimizer trajectories are exact; CPU authority, fallback, residual D2H, and component D2H remain zero. The same authoritative cache is now enabled for all supported full-CUDA CI methods. Clean strict-method receipts record hit/lookup counts of `29,544/31,325`, `25,250/26,967`, and `24,844/26,646` for `hsic.gamma`, `dcc.perm`, and `hsic.perm`, with zero bypass and identity rejection. It is disabled deterministically with `FASTKPC_PHASE10_FIXED_SP_ROOT_CACHE=0`.
>
> **Latest setup/optimizer pipeline:** The retained opt-in path keeps R and all Rcpp objects on the main thread, sends only owned optimizer inputs to one background future, and leaves CUDA handle allocation and H2D serialized with the optimizer. It preserves all original windows, setups, target cohorts, target order, and replay semantics. A level-3 delay sweep measured `114.038` seconds without the pipeline, `105.247` at 0 ms, `102.770` at 5 ms, `103.577` at 10 ms, and `104.940` at 20 ms. The default-`Inf` 5 ms `dcc.gamma` run completed in `219.513` seconds versus `233.848`, overlapping `19.319` seconds of setup work across 138 windows. The pipeline now supports all strict methods; clean `hsic.gamma/dcc.perm/hsic.perm` receipts overlap `20.167/17.538/17.641` seconds of setup work without changing physical residual-fit counts. All p-value, graph, trace, optimizer, RNG, authority, and transfer gates remain satisfied. GPU-side setup overlap was rejected after fixed-SP H2D rose from `1.469` to `38.169` seconds; CPU affinity and synchronous level tails were also wall-time neutral. The pipeline remains opt-in with `FASTKPC_PHASE10_SETUP_OPTIMIZER_PIPELINE=1` and `FASTKPC_PHASE10_SETUP_OPTIMIZER_PRODUCER_DELAY_US=5000`.

> **Guarded-Q copy-elision STOP:** A corrected prototype kept guarded-QR Q in the decomposition input workspace and used the otherwise idle left-vector buffer for derivative scratch. It preserved all 220,859 level-3 task rows and p-value bits, adjacency, sepsets, pMax, counts, semantic levels, and optimizer trajectory counters. Wall time was `114.002` seconds versus the `114.038`-second fixed-root-cache baseline, and instrumented QR/Q cycle changes were noise-sized. The alias prototype is therefore rejected as `STOP_GUARDED_Q_COPY_ELISION_WALL_TIME` and is absent from the production source and binary. Its development receipt is `fresh-data-level-03-guarded-q-alias-v2-development.rds`.
>
> **Latest objective-evaluation optimization:** Line-search trial points now execute score-only decomposition and defer gradient, Hessian, and coefficient construction until the original acceptance rule requires them. Accepted guarded-QR trials continue from the existing decomposition; accepted stable-SVD trials generate only the deferred right vectors before the same derivative path. Trial points, scores, acceptance/halving decisions, iterations, stability replay, and terminal confirmation are unchanged. On level 3, multi-penalty optimizer time fell from `40.939` to `36.883` seconds (`9.91%`) and total time from `122.849` to `118.729` seconds; derivative/Hessian cycles fell `21.31%`. The full explicit-max-7 run took `249.539` seconds versus the prior `263.305`, with all 240,489 task rows and p-value bits, adjacency, sepsets, pMax, n.edgetests, level semantics, and optimizer counters exact. Penalty multiplier hoisting and inverse-pivot lookup reduce penalty-factor cycles but were wall-time neutral by themselves. A single-matrix two-warp column-partition prototype was internally bitwise exact but throughput-flat and is removed (`STOP_SINGLE_MATRIX_TWO_WARP_COLUMN_PARTITION`). The retained source builds all capacity variants and passes the 28-case Phase 6 optimizer suite plus q64/q192/q384/q559 capacity qualification.
>
> **Latest single-matrix resource STOPs:** Three small64 prototypes preserve every one of the 220,859 level-3 task and p-value bits, all graph/level fields, and every optimizer trajectory counter, but none improves the accepted `102.770`-second pipelined level-3 baseline. Moving the augmented QR matrix to dynamic shared memory through q=46 takes `103.084` seconds; QR/SVD cycles fall `2.58%` and Q-generation cycles `5.33%`, but multi-optimizer time is flat (`36.159` to `36.137` seconds), so the decision is `STOP_SHARED_QR_MATRIX_WALL_TIME`. Fusing each left Householder dot/update removes the global work-vector round trip and intermediate warp synchronizations, but increases the optimizer thread stack from 672 to 704 bytes and takes `103.529` seconds with multi-optimizer time `36.740` seconds (`STOP_FUSED_LEFT_REFLECTOR_WALL_TIME`). Compiling the unchanged kernel with `--maxrregcount=192` lowers registers from 255 to 192 but raises the thread stack to 928 bytes and takes `104.373` seconds with multi-optimizer time `37.429` seconds (`STOP_OPTIMIZER_REGISTER_CAP_WALL_TIME`). All three prototypes are absent from the retained source and binary; their development receipts remain under `phase10_profile_v2`.
>
> **Latest dCov allocator STOP:** Replacing the nine exact-batch `cudaMalloc/cudaFree` pairs with same-stream `cudaMallocAsync/cudaFreeAsync` preserves every level-3 task bit, graph field, and level semantic. Exact dCov host time falls only from `26.844` to `26.111` seconds and teardown from `2.994` to `2.719` seconds, while fixed-SP residual time rises from `13.271` to `26.601` seconds because allocator-pool work enters the producer/consumer critical path. Total time regresses from `102.770` to `116.948` seconds. The decision is `STOP_EXACT_ASYNC_ALLOCATOR_CRITICAL_PATH`; the prototype is absent from retained source and binary. A persistent one-call context would need a bounded preallocated arena established outside both producer and consumer timing, not per-batch stream-ordered allocation.
>
> **Latest primary/replay specialization STOP:** Compile-time `<primary>` and `<stability-replay>` optimizer kernel instances preserve all level-3 task bits, graph/level fields, and trajectory counters, but do not reduce the dominant kernel's register pressure. The primary instance remains at 255 registers and its thread stack increases from 672 to 704 bytes. Level 3 takes `104.125` seconds and multi-optimizer time rises from `36.159` to `37.234` seconds. The decision is `STOP_PRIMARY_REPLAY_SPECIALIZATION_WALL_TIME`: replay-only control flow is not the source of the primary resource ceiling, and the prototype is absent from retained source and binary.
>
> **Checkpoint-B development STOP for the current contract epoch:** The retained best remains the exact `219.513`-second default-`Inf` pipeline receipt, `39.513` seconds above the `<180` gate. Even granting the entire measured dCov orchestration upper bound (`3.459` seconds teardown plus `7.416` seconds otherwise unattributed host time) as free leaves `208.638` seconds and a `28.638`-second optimizer-only deficit. The setup pipeline already overlaps `19.319` seconds; moving CUDA allocation/H2D into the producer raised fixed-SP H2D to `38.169` seconds and cannot supply the balance. Score-first evaluation is the only retained objective-internal improvement. Grouped QR (`1.032x`), DGER/block/launch tuning, Q copy elision, shared QR, fused reflectors, register capping, and primary/replay specialization are all exact but wall-time insufficient or regressive. The decision is `STOP_PHASE10_STRICT_180_NO_QUALIFIED_ROUTE`: do not freeze a candidate, open the holdout, or recommend the route. This is not a hardware impossibility claim. Reopen only after an isolated, real-shape single-matrix evaluator demonstrates at least `1.5x` QR/Q throughput and an expected full multi-optimizer speedup of at least `1.35x`, or after explicitly defining and qualifying a new numerical-contract epoch.
>
...
>
> **Latest fixed-SP residual attribution:** The trace-free full level-7 compute-profile v6 takes `274.728` seconds as a development diagnostic and does not replace the `235.058`-second explicit-max-7 best runtime. Its graph, sepsets, counts, 240,489 task rows including consumed p-values, and semantic level trace are bitwise identical to the primitive-cache artifact, with zero CPU numerical authority, fallback, residual D2H, or component D2H. Authoritative fixed-SP residual solve time is `15.315` seconds: single-penalty `10.272`, multi-penalty `5.043`, exact screen `14.833`, and guarded legacy-eig refinement only `0.482`. The run executes 38,613 exact residual batches and 553 refinement batches; refinement is only `3.15%` of residual time, so exact/refinement residual sharing is below the `3-5` second implementation gate and stops as a main route. Exact screens nevertheless perform 228,015 residual target fits for 110,665 unique ResidualKeys, leaving 117,350 excess fits (`51.47%`). That apparent target-level opportunity motivated the cross-batch qualification recorded below; it was not treated as reusable without evidence. The same run records 39,166 dCov calls, `3.440` seconds of teardown, and `7.416` seconds of otherwise unattributed dCov host time; this is only an upper bound for a persistent dCov execution context.
>
> **Latest fixed-SP cross-batch qualification:** Device-only repeat/permutation/subset/singleton/route-mix fixtures show that complete identical or permuted cohorts are bitwise exact, but batched-Cholesky miss-only subsets are not. Across the fixtures there are 913 residual-value mismatches, 332,530 exact-component value/moment mismatches, three solver-status mismatches, and two unconditional legacy-eig p-value mismatches. Final guarded p-values and decisions remain unchanged, but the target-granular strict gate fails; the intermediate decision is `CONDITIONAL_ALL_HIT_BATCH_ONLY`. A trace-free full level-7 opportunity run then classifies all 38,613 exact batches and 228,015 target fits with exact accounting. Only 46 batches and 1,853 targets are both all-hit and a repeated complete cohort. Their measured residual solve plus exact-component upper bound is only `71.354` ms (`54.209 + 17.145`), below the 5-second stop gate. The final decision is `STOP_CROSS_BATCH_FIXED_RESIDUAL_CACHE_OPPORTUNITY`: do not implement target-granular caching, residual slabs, exact-component caching, capacity traces, or whole-cohort reuse in this contract epoch. The diagnostic run takes `266.801` seconds and does not replace the `235.058`-second explicit-max-7 baseline; its graph/task outputs and semantic level fields remain bitwise exact with zero CPU authority, fallback, or large-payload D2H.
>
> **Default-Inf semantic and capacity qualification:** `kpcalg::kpc()` defaults to `m.max = Inf`; for finite `p`, the search ceiling is `p - 2` and the graph should stop naturally. On the canonical 351x48 graph, default Inf requires nine level-8 tests, giving counts `2213,52659,125293,40694,13293,5422,835,80,9`, and then naturally stops. The one-call route resolves Inf to 46 and dispatches multi-penalty CUDA work through `q/penalty` capacity buckets `64/7`, `80/8`, `192/21`, `384/42`, and `559/62`, with bounded concurrency `64,64,8,2,1`. The q192 level-9 fixture has CPU optimizer-trajectory qualification; q384 and the maximum `n=600,p=64,|S|=62,q=559` shape have CUDA authority/capacity qualification. With score-first evaluation and authoritative fixed-SP root caching, the default-Inf development run completed in `233.848` seconds, down from `252.616` and the Phase 9 v2 `277.868`-second artifact. Its 240,498 task rows and every p-value bit, adjacency, sepsets, pMax, n.edgetests, optimizer trajectory, and level semantics are exact against the older authority artifacts, with zero CPU authority, fallback, residual D2H, or component D2H. The public 500x50 v2 fixture also uses default Inf and stops naturally at level 3. Checkpoint B still fails, and this single development run is not a frozen candidate or five-run median.
>
...
>
> **Strict-method optimization pass:** A route-authenticated one-call residual slab, persistent method CUDA context, and 256-thread HSIC row kernel retain every original miss cohort and gather only complete all-hit residual batches D2D. Conditioning systems through two penalties use qualified augmented QR; larger systems retain authoritative stable SVD. `hsic.perm` keeps a bounded 384 MiB deterministic component LRU and exact R 4.4 Rejection index generation. Commit `830cfbe` adds the authoritative fixed-SP root cache and opt-in original-window setup/optimizer pipeline to all strict methods. Its C++17 move-only permutation builder owns the table, computes OpenSSL SHA-256 incrementally, seals the payload without a mutable consumer alias, and eliminates the trusted consumer's second full payload scan; untrusted boundaries still rehash. Clean default-Inf 351x48 outer times at that point were `593.814/849.693/1042.358` seconds for `hsic.gamma/dcc.perm/hsic.perm`, down from `631.522/945.359/1152.905` at `50595ce`. Commit `6c01d71` then submits one strict-method GPU preparation before each permutation build, retains one in-flight ticket, queues finalization on the same stream, and preserves delayed-error/RNG semantics. New clean canonical times are `839.726` seconds for `dcc.perm` and `950.008` seconds for `hsic.perm`, saving `9.967` (`1.17%`) and `92.350` (`8.86%`) seconds; `hsic.gamma` was not rerun and remains referenced at `593.814` seconds. Physical residual fits remain `194,687/189,896/189,258`. `hsic.gamma` retains maximum p-value difference `8.271161533457416e-14` under its `1e-10` contract with zero decision flips; both permutation routes remain bitwise exact, including final R RNG state. All relevant skeleton tests, kpcalg-authority orientation CI tests, traces, graph outputs, and final PDAGs pass; CPU numerical authority, fallback, residual/component D2H, hidden submit synchronization, intermediate host waits, payload rescans, residual-cache eviction, root-cache bypass, and root identity rejection are zero. The overlap native SHA-256 is `88eb8939759e6beb119126b06a2a110d2f7ca44ccda4333c8fef0109bc5fa59b`; evidence is indexed by `fastkpc/artifacts/strict_ci_methods_351x48_permutation_overlap_v1/manifest.json`, with the unchanged `hsic.gamma` evidence retained in optimized v2. This does not change the `dcc.gamma` Phase 10 candidate, sealed holdout, or promotion state.
>
> **Strict-method remaining opportunity:** The optimized routes are not at a hardware ceiling. Authoritative fixed-SP residual solving now costs `383.800/452.323/429.871` seconds for `hsic.gamma/dcc.perm/hsic.perm`, dominated by level-3-and-deeper stable SVD. The one-in-flight permutation/GPU pipeline is implemented and qualified, but concurrent permutation plus incremental digest grows to `139.876` seconds for `dcc.perm` and `278.271` seconds for `hsic.perm`. Every `dcc.perm` preparation is ready by the end of permutation generation, while submit itself accumulates `459.429` seconds because QR cohorts retain internal waits; that route therefore realizes only a small 9.967-second wall-time improvement. `hsic.perm` realizes a material 92.350-second improvement despite concurrent CPU slowdown. The next shared high-impact route is a qualified deeper-level residual kernel; reducing QR submit waits may be useful specifically for `dcc.perm`. Mixed-hit masking remains diagnostic-only, and more scheduler, target-cache, or outer grouped-QR work remains stopped by existing evidence.
>
...
proof and public 500x50 fixture pass. Lazy multi-penalty handles, per-call
univariate primitive reuse, score-first trial evaluation, and authoritative
fixed-SP root caching reduce the explicit-max-7 development profile to
`235.058` seconds and default Inf to `233.848` seconds with exact numerical and
graph parity, but Checkpoint B and the final five-run gate still fail. A
non-formal
