...
>
> **Strict-method residual/component optimization:** A one-call-scoped, route-authenticated residual slab now skips only complete all-hit fixed-SP SVD batches; every batch containing a miss retains its original full cohort, while hits are gathered D2D in original target order. The bounded cache used 368,229,888 device bytes with zero eviction on all three canonical runs. Independent row work in the strict HSIC incomplete-Cholesky component kernel is now distributed over 256 threads while pivoting, triangular solves, means, and reductions retain their original order. Full default-Inf 351x48 outer elapsed times improved from `1562.052` to `979.870` seconds for `hsic.gamma` (`1.594x`), from `2131.261` to `1455.935` seconds for `dcc.perm` (`1.464x`), and from `3877.316` to `1875.133` seconds for `hsic.perm` (`2.068x`). Physical residual fits fell from `308,199/459,350/564,226` to `194,687/189,896/189,258`. The `hsic.gamma` component segment fell from `233.559` to `34.586` seconds; the `hsic.perm` component segment fell from `1155.125` to `172.454` seconds. All 834,467 p-values preserve the established contracts, with exact logical/deletion traces, graph outputs, permutation RNG terminal states, kpcalg-authority orientation CI, and final PDAGs; CPU numerical authority, fallback, residual D2H, component D2H, and cache eviction are all zero. This closes the first production optimization pass for the three opt-in methods but does not change the `dcc.gamma` Phase 10 candidate or promotion state.
>
