...
>
> **Strict-method optimization pass:** A route-authenticated one-call residual slab, persistent method CUDA context, and 256-thread HSIC row kernel retain every original miss cohort and gather only complete all-hit residual batches D2D. Conditioning systems through two penalties now use the fully qualified augmented-QR residual route; larger systems retain authoritative stable SVD. `hsic.perm` adds a bounded 384 MiB deterministic component LRU and an exact inline implementation of R 4.4's Rejection `R_unif_index()` path for `n <= 32768`, with R-API fallback outside that contract. Permutation request identities retain SHA-256 over every table byte, but the production builder now uses OpenSSL SHA-256 without an intermediate payload string copy. Full default-Inf 351x48 outer elapsed times are `631.522` seconds for `hsic.gamma` (from `1562.052`, `2.473x`), `945.359` for `dcc.perm` (from `2131.261`, `2.254x`), and `1152.905` for `hsic.perm` (from `3877.316`, `3.363x`). Physical residual fits are `194,687/189,896/189,258`. The final `hsic.gamma` maximum p-value difference is `8.271161533457416e-14` under its `1e-10` contract with zero decision flips; both permutation routes remain bitwise exact, including final R RNG state. All 834,467 skeleton tests, 19 kpcalg-authority orientation CI tests, logical/deletion traces, graph outputs, and final PDAGs pass; CPU numerical authority, fallback, residual D2H, component D2H, and residual-cache eviction are zero. The `hsic.perm` component LRU records 240,685 hits, 323,541 misses, and 234,199 bounded deterministic evictions. These method extensions do not change the `dcc.gamma` Phase 10 candidate, sealed holdout, or promotion state.
>
> **Strict-method remaining opportunity:** The optimized routes are not at a hardware ceiling. Authoritative fixed-SP residual solving still costs `383.057/455.455/439.446` seconds for `hsic.gamma/dcc.perm/hsic.perm`, dominated by level-3-and-deeper stable SVD. `hsic.perm` also spends `185.970` seconds generating 9,902,166,300 permutation indices and `135.129` seconds in the two SHA-256 authentication boundaries; `dcc.perm` spends `61.091` and `108.851` seconds respectively. The next material routes are a strictly qualified deeper-level residual kernel and a permutation generation/authentication pipeline that preserves exact tables and R RNG terminal state. More scheduler, target-cache, or outer grouped-QR work remains stopped by the existing Phase 10 evidence.
>
